...
int add_to_data(char file_name[])
{
    ...
    s_product new_product;

    printf("Enter product's name: ");
    check = enter_string(new_product.name, NAME_LEN);
    if (check != EXIT_SUCCESS)
        return check;

    printf("Enter manufacturer's name: ");
    check = enter_string(new_product.manufacturer, MANUFACTURER_LEN);
    if (check != EXIT_SUCCESS)
        return check;
    ...
    fwrite(&new_product, 1, sizeof(new_product), file);
